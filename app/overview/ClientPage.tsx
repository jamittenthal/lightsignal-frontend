...
async function fetchData(companyId?: string) {
  try {
    const resp = await callIntent("financial_overview", { action: "load" }, companyId || "demo");
...
export default async function ClientPage({ companyId }: Props) {
  const data: FinancialData = await fetchData(companyId);
  const k = data.kpis || {};

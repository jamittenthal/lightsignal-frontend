...
function ScenariosContent({ companyId }: { companyId?: string }) {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      try {
        const result = await fetchScenarioFull({ company_id: companyId || "demo" });
        setData(result);
      } catch (error) {
...
export default function ClientPage({ companyId }: Props) {
  return (
    <Suspense fallback={
      ...
      <ScenariosContent companyId={companyId} />
    </Suspense>
  );
}